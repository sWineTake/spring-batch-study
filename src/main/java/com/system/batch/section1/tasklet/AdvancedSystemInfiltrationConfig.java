...
public class AdvancedSystemInfiltrationConfig {
    ...
    public Job systemInfiltrationJob(JobRepository jobRepository, Step reconStep, Step attackStep) {
        // JobBuilder 의 listener 메서드로 등록해주면 beforeJob() 메서드에서 동적으로 생선한 데이터를 각 Step에서 참조할 준비가 완료된다.
    ...
    public Step reconStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder("reconStep", jobRepository)
                .tasklet(
                    ...
                    // infiltrationPlanListener 에서 BeforeJob에서 생성한 동적 데이터를 가져옴
                    Map<String, Object> infiltrationPlan =
                        (Map<String, Object>) chunkContext.getStepContext().getJobExecutionContext().get("infiltrationPlan");
    ...
    public Step attackStep(
        JobRepository jobRepository, PlatformTransactionManager transactionManager, Tasklet attackStepTasklet  // 주입받은 Tasklet 사용
    ) {
